...
data = load('sensorlog_spun_all_axis.mat');
nrbin = 50; % number of bins

T = 1/100; %1/f 100 Hz
N = length(table2array(data.MagneticField)); % no of points
...
beta = 0.5; % the limit for assuming that an acceleration is happening in 
             % Outliner 1, how different the acceleration is between the 
             % gyro and acc can be
             % 0.05 best for outliner 1
             % 0.5 best for outliner 2
...
for i = 2:n
    ...
    acc_x_diff = gyro_x_diff/T - acc(i-1,1);
    acc_y_diff = gyro_y_diff/T - acc(i-1,2);
    acc_z_diff = gyro_z_diff/T - acc(i-1,3) - g;
    

    if  acc_x_diff < beta || acc_y_diff < beta || acc_z_diff < beta
        acc_edit(i-1,:) = acc(i-1,:);
    else
        acc_edit(i-1,:) = [gyro_x_diff/T, gyro_y_diff/T, gyro_z_diff/T + g];
    end
